...
class NeuronalNetwork(object):
    ...
    def __init__(self, saveFile=None, layerCount=0, layerSize=None, actFunctions=None):
        ...
        if saveFile:
            ...
            for i in range(1, len(layerSize) - 1):
                r = 2 * \
                    np.random.random(
                        (layerSize[i - 1] + 1, layerSize[i] + 1)) - 1
    ...
    def test(self, testimages):
        numtests = testimages.shape[1]
        outputs = []
        for i in range(numtests):
            outputs.append(self._test(testimages[:, i].T))
